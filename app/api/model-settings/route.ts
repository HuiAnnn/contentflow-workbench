...
export async function GET(){
 try{const user=await getWorkspaceIdentity();if(!user)return json({error:'工作区会话已过期，请刷新页面'},401);return json(await getModelConfiguration(user.userId))}
 catch{return json({error:'连接列表暂时无法读取，请稍后重试。'},503)}
}
export async function POST(request:Request){
 try{
  const user=await getWorkspaceIdentity();if(!user)return json({error:'工作区会话已过期，请刷新页面'},401);
